...
#define RANGE_EPSILON 5 //区间限定符


extern bool multiline;
...
struct Rule {
    ...
    Rule(int _dst, RuleType _type, int _lower, int _upper) : dst(_dst), type(_type), lower(_lower), upper(_upper) {}
    bool match(char c);
};
...
public:
    ...
    int groups = 0;                       //第三次实验用到，记录分组的数目
    std::vector<int> marks_start;         //第三次实验用到，标记各个分组的起始位置。
    std::vector<int> marks_end;           //第三次实验用到，标记各个分组的结束位置。
    /**
     * 在自动机上执行指定的输入字符串。
    ...
    std::vector<std::string> exec_2(std::string const_text, std::string text, int length);
    void DFS(int q, std::string const_string, std::string text, int step, int length);
