...
std::string NFA::exec_2(std::string text) {
    // TODO 请你完成这个函数
    DFS(0, text, 0);
    std::string result_string;
    int result_string_size = text.length() - path_record[path_record.size() - 1].remain.size();
    result_string = text.substr(0, result_string_size);  //去掉remain的就是用掉的
